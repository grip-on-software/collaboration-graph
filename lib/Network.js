...
function resized() {
    ...
    if ((oldWidth <= TABLET_WIDTH) != (width <= TABLET_WIDTH)) {
        svg.selectAll("circle").attr("r", d => getNodeRadius(d));
        simulation.force("link")
            .distance(() => width <= TABLET_WIDTH ? 18 : 30);
    }

    simulation
        .force("charge", d3.forceManyBody().strength(node =>
            (width <= TABLET_WIDTH ? 0.4 : 1) *
            (node.colorIndex > 1 ? -30 : -300)
        ))
        .force("center", d3.forceCenter(width / 2, height / 2))
        .force("x", d3.forceX(width / 2).strength(0.05 * height / width))
        .force("y", d3.forceY(height / 2).strength(0.05 * width / height))
        .force("circle", d3.forceRadial(10, width / 2, height / 2)
            .strength(0.05))
        .alphaTarget(0.3)
        .restart();
}
...
function getNodeRadius(d) {
    ...
    if (d.colorIndex <= 1) {
        return width <= TABLET_WIDTH ? 6 : 10;
    }

    return width <= TABLET_WIDTH ? 3 : 5;
}
